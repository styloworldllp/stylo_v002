...
def capture_user_persona(responses: str):
	...
	# Persona capture disabled — external Frappe endpoint removed
	frappe.db.set_single_value("LMS Settings", "persona_captured", True)
	return {"message": {"name": "disabled"}}
...
def clear_demo_data():
	frappe.only_for("Moderator")
	quiz_title = "Do you know Stylo LMS?"
	if frappe.db.exists("LMS Quiz", {"title": quiz_title}):
		frappe.db.delete("LMS Quiz", {"title": quiz_title})

	demo_course = frappe.get_all("LMS Course", {"title": "A guide to Stylo LMS"}, pluck="name")
