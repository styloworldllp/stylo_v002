...
frappe.ui.form.on("Server Script", {
	...
	check_safe_exec(frm) {
		frappe.xcall("frappe.core.doctype.server_script.server_script.enabled").then((enabled) => {
			if (enabled === false) {
				let docs_link =
					"https://styloworld.io/docs/server-script";
	...
	setup_help(frm) {
		...
		help_field.html(`
<h4>DocType Event</h4>
<p>Add logic for standard doctype events like Before Insert, After Submit, etc. Use <code>stylo</code> (or <code>frappe</code>) as the namespace.</p>
...
<p>Payment processing events have a special state.</p>
<pre><code class="language-python">
# retrieve payment session state
...
customer.customer_name = doc.first_name + " " + doc.last_name
